...
// -----------------------------------------------------------------------------
// Latest user‑provided version (requestVideoFrameCallback + LERP, FP32 path)
// -----------------------------------------------------------------------------
(() => {
  /* === CONFIG === */
  ...
  wk.onmessage=e=>{
    if(e.data.type==='ready'){
      ...
      lastBoxes=e.data.boxes; // 参照渡し
      pending=false;
    }
  };

  // FP32 model (Safari iOS needs FP32)
  wk.postMessage({type:'init',modelUrl:'teeth_v12n_fp32.onnx'});
  ...
  /* === Render loop via requestVideoFrameCallback === */
  let pending=false,lastBoxes=null,smoothBoxes=null;
  async function onFrame(now,meta){
    if(video.readyState>=2&&workerReady){
      ...
      // ボックス描画
      if(lastBoxes){
        if(!smoothBoxes||smoothBoxes.length!==lastBoxes.length){smoothBoxes=new Float32Array(lastBoxes);}else{for(let i=0;i<lastBoxes.length;i++){smoothBoxes[i]+=SMOOTH*(lastBoxes[i]-smoothBoxes[i]);}}
        const sL=Math.min(INPUT/sw,INPUT/sh);const dwL=sw*sL,dhL=sh*sL,dxL=(INPUT-dwL)/2,dyL=(INPUT-dhL)/2;
        ctx.lineWidth=3;ctx.strokeStyle='red';ctx.fillStyle='yellow';ctx.font='14px sans-serif';
        for(let i=0;i<smoothBoxes.length;i+=6){const conf=smoothBoxes[i+4];if(conf<TH)continue;const vx1=(smoothBoxes[i]-dxL)/sL,vy1=(smoothBoxes[i+1]-dyL)/sL,vx2=(smoothBoxes[i+2]-dxL)/sL,vy2=(smoothBoxes[i+3]-dyL)/sL;const x1=layout.offsetX+vx1*layout.scale,x2=layout.offsetX+vx2*layout.scale,y1=layout.offsetY+vy1*layout.scale,y2=layout.offsetY+vy2*layout.scale;if(y1<layout.detectTop||y2>layout.detectTop+canvas.width)continue;ctx.strokeRect(x1,y1,x2-x1,y2-y1);ctx.fillText((conf*100).toFixed(1)+'%',x1+4,y1+16);} }
    }
