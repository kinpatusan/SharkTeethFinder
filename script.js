...
(() => {
  ...
  async function populateCamSel(){const cams=await listCams();const rear=cams.filter(c=>/rear|back|背面/i.test(c.label));const wide=rear[0]||cams[0];const ultra=rear.find(c=>/ultra[- ]?wide|超広角/i.test(c.label));camSel.innerHTML='';const add=(c,l)=>{if(!c)return;const o=document.createElement('option');o.value=c.deviceId;o.textContent=l;camSel.appendChild(o);};add(wide,'背面カメラ');add(ultra,'背面超広角');camSel.disabled=camSel.options.length<=1;}
  async function setupCamera(deviceId){if(currentStream)currentStream.getTracks().forEach(t=>t.stop());const constr=deviceId?{video:{deviceId:{exact:deviceId}},audio:false}:{video:{facingMode:'environment'},audio:false};currentStream=await navigator.mediaDevices.getUserMedia(constr);video.srcObject=currentStream;await video.play();layout.update(video.videoWidth,video.videoHeight);}
  ...
  /* === Init === */
  (async()=>{status.textContent='Requesting camera…';try{await setupCamera();}catch(e){status.textContent='Camera error';console.error(e);return;}await populateCamSel();status.textContent='Loading model…';loop();})();
})();
