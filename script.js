...
(() => {
  ...
  /* === Worker input === */
  const tmp=document.createElement('canvas');tmp.width=tmp.height=1088;const tctx=tmp.getContext('2d');function drawLetterbox(){const sw=video.videoWidth,sh=video.videoHeight,s=Math.min(1088/sw,1088/sh);const dw=sw*s,dh=sh*s,dx=(1088-dw)/2,dy=(1088-dh)/2;tctx.drawImage(video,0,0,sw,sh,dx,dy,dw,dh);}

  /* === Render loop === */
  let pending=false,lastBoxes=null;
  async function loop(){if(video.readyState>=2&&workerReady){const sw=video.videoWidth,sh=video.videoHeight;layout.update(sw,sh);if(!pending){drawLetterbox();const bmp=await createImageBitmap(tmp);wk.postMessage({type:'frame',bitmap:bmp},[bmp]);pending=true;}ctx.drawImage(video,0,0,sw,sh,layout.offsetX,layout.offsetY,sw*layout.scale,sh*layout.scale);if(lastBoxes){const sL=Math.min(1088/sw,1088/sh);const dwL=sw*sL,dhL=sh*sL,dxL=(1088-dwL)/2,dyL=(1088-dhL)/2;ctx.lineWidth=3;ctx.strokeStyle='red';ctx.fillStyle='yellow';ctx.font='14px sans-serif';for(let i=0;i<lastBoxes.length;i+=6){const conf=lastBoxes[i+4];if(conf<TH)continue;const vx1=(lastBoxes[i]-dxL)/sL,vy1=(lastBoxes[i+1]-dyL)/sL,vx2=(lastBoxes[i+2]-dxL)/sL,vy2=(lastBoxes[i+3]-dyL)/sL;const x1=layout.offsetX+vx1*layout.scale,x2=layout.offsetX+vx2*layout.scale,y1=layout.offsetY+vy1*layout.scale,y2=layout.offsetY+vy2*layout.scale;if(y1<layout.detectTop||y2>layout.detectTop+canvas.width)continue;ctx.strokeRect(x1,y1,x2-x1,y2-y1);ctx.fillText((conf*100).toFixed(1)+'%',x1+4,y1+16);}}}requestAnimationFrame(loop);}
