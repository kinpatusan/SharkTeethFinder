...
// +2025-06-23  Switched rAF loop → requestVideoFrameCallback (GPU load ↓)
// -----------------------------------------------------------------------------
(() => {
  /* === DOM === */
  ...
  async function pump(now,meta){
    if(video.readyState>=2&&workerReady){
      ...
      if(lastBoxes){
        const sL=Math.min(1088/sw,1088/sh);
        const dwL=sw*sL,dhL=sh*sL,dxL=(1088-dwL)/2,dyL=(1088-dhL)/2;
        ctx.lineWidth=3;ctx.strokeStyle='red';ctx.fillStyle='yellow';ctx.font='14px sans-serif';
        for(let i=0;i<lastBoxes.length;i+=6){
          const conf=lastBoxes[i+4];
          if(conf<TH)continue;
          const vx1=(lastBoxes[i]-dxL)/sL,vy1=(lastBoxes[i+1]-dyL)/sL,vx2=(lastBoxes[i+2]-dxL)/sL,vy2=(lastBoxes[i+3]-dyL)/sL;
