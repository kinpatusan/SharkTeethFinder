...
// -----------------------------------------------------------------------------
// 2025‑06‑25  Portrait 4:3 full‑width, 1:1 detect, top/bottom mask – **width‑fit fix**
//   • ビデオは「横幅ぴったり」にフィットさせ、左右黒帯ゼロ
//   • 暗帯は検知正方形の上下のみ（映像透過）
//   • バウンディングボックスを widthFit スケールで再計算
// -----------------------------------------------------------------------------
(() => {
  /* === DOM === */
  ...
  async function loop(){
    if(video.readyState>=2 && workerReady){
      const sw=video.videoWidth, sh=video.videoHeight;
      layout.update(sw,sh);
      // 1. send frame to worker
      if(!pending){drawLetterbox();const bmp=await createImageBitmap(tmp);wk.postMessage({type:'frame',bitmap:bmp},[bmp]);pending=true;}
      // 2. background (width‑fit)
      const drawnH = sh * layout.scale;
      ctx.drawImage(video,0,0,sw,sh,layout.offsetX,layout.offsetY,canvas.width,drawnH);
      // 3. draw boxes mapped with widthFit scale
      if(lastBoxes){
